...
// Query baru untuk mengambil data film dan menggabungkan nama genre
$stmt = $connect->prepare("
    SELECT 
        v.*, 
        GROUP_CONCAT(g.nama SEPARATOR ' | ') AS genre_names
    FROM 
        videos v 
    LEFT JOIN 
        video_genres vg ON v.id = vg.video_id
    LEFT JOIN 
        genres g ON vg.genre_id = g.id
    WHERE 
        v.id = ?
    GROUP BY
        v.id
        ...
        <div class="row">
            ...
            <div class="col-md-8 col-lg-9" data-aos="fade-left">
                ...
                <div class="d-flex align-items-center my-3 text-secondary">
                    ...
                    <span><?= htmlspecialchars($film['genre_names'] ?? 'Movie') ?></span>
                    <span class="mx-2">|</span>
                ...
                <?php else: ?>
                    <button type="button" class="btn btn-danger btn-lg mt-3" data-bs-toggle="modal" data-bs-target="#loginModal">
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content bg-dark">
      ...
      <div class="modal-body">
        ...
        <p class="text-center mt-3">Don't have an account? <a href="/auth/register.php?redirect=<?= urlencode($_SERVER['REQUEST_URI']) ?>" class="text-danger">Register here</a></p>
      </div>
    </div>
  </div>
</div>
